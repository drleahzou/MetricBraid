...
the contract between the routing rules in [`CLAUDE.md`](../CLAUDE.md) and
anything that reports a number to a person.

Machine-readable form: [`routed-observation.schema.json`](routed-observation.schema.json).
...
- [`examples/passive-conflict.json`](examples/passive-conflict.json) — two
